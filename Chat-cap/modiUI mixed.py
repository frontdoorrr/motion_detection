...
class WindowClass(QMainWindow, form_class1) :
    def __init__(self,proffessor_window, student_window) :
        super().__init__()
        self.setupUi(self)
        widget.setFixedHeight(400)
        widget.setFixedWidth(400)

        self.proffessor_window = proffessor_window
    ...
    def make_room_function(self):
        ...
        USER_DATA.room_number = self.get_room_number.text()
        USER_DATA.username = self.get_username.text()
        ...
        self.proffessor_window.room_number_lbl.setText(USER_DATA.room_number)
        self.proffessor_window.username_lbl.setText(USER_DATA.username)
    ...
    def enter_room_function(self) :
        ...
        USER_DATA.room_number = self.get_room_number.text()
        USER_DATA.username = self.get_username.text()
        print(USER_DATA.room_number, USER_DATA.username)
        widget.setFixedHeight(200)
        widget.setFixedWidth(400)
        self.student_window.room_number_lbl.setText(USER_DATA.room_number)
        self.student_window.username_lbl.setText(USER_DATA.username)
        ...
        with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
            while cap.isOpened():
                ...
                try:
                    ...
                    USER_DATA.motion_msg = body_language_class
...
class StudentClass(QMainWindow,form_class3) :
    def __init__(self) :
        ...
        self.room_number_lbl.setText(USER_DATA.room_number)
        self.username_lbl.setText(USER_DATA.username)

        self.timer_one = QTimer(self)
        self.timer_one.start(300)
        self.timer_one.timeout.connect(self.initUI2)

    def initUI2(self):
        if USER_DATA.motion_msg == 'HandL' or USER_DATA.motion_msg == 'HandR':
            self.motion_client.setPixmap(QPixmap("handup.jpg"))
        elif USER_DATA.motion_msg == 'PoseO':
            self.motion_client.setPixmap(QPixmap("O_img.jpg"))
        elif USER_DATA.motion_msg == 'PoseX':
        ...
        # print(self.room_number_lbl,self.username_lbl)



#화면을 띄우는데 사용되는 Class 선언 2 교수자용 화면
class ProffessorClass(QMainWindow,form_class2) :
...
if __name__ == "__main__" :
    ...
    proffessor_window = ProffessorClass()
    student_window = StudentClass()
    main_window = WindowClass(proffessor_window, student_window)

    #Widget 추가
    widget.addWidget(main_window)
    widget.addWidget(proffessor_window)
